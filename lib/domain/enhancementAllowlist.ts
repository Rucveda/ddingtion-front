import { WILD_BASE, WILD_SPECIAL } from "@/lib/domain/marketData";
...
const WILD_SPECIAL_NAMES = new Set(
  WILD_SPECIAL.map(([name]) => normalizeOptionKey(name as string))
);

export type WildEnchantDisplayVariant = "normal" | "special" | "high-tier";

export function getWildEnchantDisplayVariant(
  name: string,
  level: number
): WildEnchantDisplayVariant {
  const key = normalizeOptionKey(name);
  if (WILD_SPECIAL_NAMES.has(key)) return "special";
  const threshold = getHighEnchantThreshold(name);
  if (threshold !== undefined && level > threshold) return "high-tier";
  return "normal";
}

const WILD_ENCHANT_BADGE_ACTIVE: Record<WildEnchantDisplayVariant, string> = {
  normal:
    "bg-blue-600 border-blue-400 text-white shadow-lg shadow-blue-600/10",
  special:
    "bg-red-600 border-red-400 text-white shadow-lg shadow-red-600/10",
  "high-tier":
    "bg-orange-600 border-orange-400 text-white shadow-lg shadow-orange-600/10",
};

export function getWildEnchantActiveBadgeClass(
  name: string,
  level: number
): string {
  return WILD_ENCHANT_BADGE_ACTIVE[getWildEnchantDisplayVariant(name, level)];
}

const WILD_ENCHANT_LIST_BADGE: Record<WildEnchantDisplayVariant, string> = {
  normal:
    "rounded-lg border border-blue-500/20 bg-blue-500/10 px-2 py-0.5 text-[11px] font-semibold text-blue-200",
  special:
    "rounded-lg border border-red-500/20 bg-red-500/10 px-2 py-0.5 text-[11px] font-semibold text-red-200",
  "high-tier":
    "rounded-lg border border-orange-500/20 bg-orange-500/10 px-2 py-0.5 text-[11px] font-semibold text-orange-200",
};

export function getWildEnchantListBadgeClass(name: string, level: number): string {
  return WILD_ENCHANT_LIST_BADGE[getWildEnchantDisplayVariant(name, level)];
}

export function isWildSpecialEnchant(name: string): boolean {
  return WILD_SPECIAL_NAMES.has(normalizeOptionKey(name));
}

export function partitionWildEnchantments(
  enchantments: Record<string, number> | null | undefined
): { base: [string, number][]; special: [string, number][] } {
  const base: [string, number][] = [];
  const special: [string, number][] = [];
  if (!enchantments) return { base, special };
  for (const [name, level] of Object.entries(enchantments)) {
    const lv = Number(level);
    if (!lv) continue;
    if (isWildSpecialEnchant(name)) special.push([name, lv]);
    else base.push([name, lv]);
  }
  return { base, special };
}
