...
const RPG_SKILLS = ["리프시커", "바인크리프", "우드서지", "버던트메테오", "그로브클랩", "스틸임팩트", "헤비사이클론", "그랜드크러시", "오리진이지스", "팔라딘저지먼트", "에너지버스트", "브로드샷", "락온트리거", "펄스레이닝", "오버클락프로토콜", "차지블로우", "스위프트샷", "컨비전스스플릿", "리니어레인", "세라핌디센트", "피어스폴", "스러스트러시", "플리커랜서", "프로스트드롭", "앱솔루트도미니온", "플래임슬래시", "리버스커터", "업리프트임팩트", "드래곤이그니션", "와이번어웨이크"];
      ...
      <nav className="sticky top-0 z-50 border-b border-white/5 bg-black/40 backdrop-blur-2xl">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center relative z-10">
          ...
          <div className="flex items-center gap-4">
            <span className="text-[11px] font-black text-zinc-500 uppercase tracking-widest hidden md:block"></span>
              ...
              <div className="space-y-6">
                <div className="relative">
                  ...
                  <AnimatePresence>
                    {showDropdown && searchTerm && (
                      <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute top-full left-0 w-full mt-2 bg-[#0d0d0f] border border-white/10 rounded-2xl overflow-hidden z-[100] shadow-3xl max-h-60 overflow-y-auto custom-scrollbar backdrop-blur-3xl">
                        {filteredItems.map(item => (
                          <div key={item.id} onClick={() => handleSelectItem(item)} className="p-4 hover:bg-white/5 cursor-pointer flex items-center gap-4 border-b border-white/5 group transition-colors">
                            <img src={item.iconUrl} className="w-8 h-8 pixel-art" alt="" />
                            <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-black/40 p-5 rounded-2xl border border-white/5">
                    ...
                    <div className="flex items-baseline gap-2">
                      <input type="text" className="w-full bg-transparent text-2xl font-mono font-black text-blue-500 outline-none placeholder:text-blue-900/20" placeholder="OPTIONAL" value={form.buyNowPrice} onChange={e => setForm({...form, buyNowPrice: e.target.value.replace(/[^0-9]/g, "")})} />
                ...
                >
                  <div className="flex items-center gap-6 mb-10 bg-white/[0.03] p-6 rounded-3xl border border-white/5">
                    <div className="w-16 h-16 bg-black/40 rounded-xl flex items-center justify-center border border-white/5 shrink-0">
                      <img src={selectedItem.iconUrl} className="w-10 h-10 pixel-art" alt="" />
                    </div>
                    <div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                            {WILD_BASE.map(([name, max]) => (
                              <button key={name as string} type="button" onClick={() => toggleOption('enchantments', name as string, max as number)} 
                                className={`flex items-center justify-between p-3.5 rounded-xl border transition-all ${form.enchantments[name as string] ? "bg-blue-600 border-blue-400 text-white" : "bg-white/5 border-transparent text-zinc-500 hover:bg-white/10"}`}>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                            {WILD_SPECIAL.map(([name, max]) => (
                              <button key={name as string} type="button" onClick={() => toggleOption('enchantments', name as string, max as number)}
                                className={`flex items-center justify-between p-3.5 rounded-xl border transition-all ${form.enchantments[name as string] ? "bg-red-600 border-red-400 text-white" : "bg-white/5 border-transparent text-zinc-500 hover:bg-white/10"}`}>
                                <span className="font-bold text-xs">{name as string}</span>
                                {form.enchantments[name as string] && <span className="font-black text-[10px] bg-white/20 px-1.5 py-0.5 rounded-md">{form.enchantments[name as string]}</span>}
                              </button>
                            ))}
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              {[0, 1, 2].map(idx => (
                                <button key={idx} type="button" onClick={() => { triggerHaptic(); setActiveRuneSlot(activeRuneSlot === idx ? null : idx); }} className={`relative aspect-square rounded-xl border flex flex-col items-center justify-center transition-all ${activeRuneSlot === idx ? 'bg-orange-500/10 border-orange-500' : 'bg-black/40 border-white/5 hover:border-white/10'}`}>
                                  {form.runes[idx].type ? (
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                            {RPG_SKILLS.map(name => (
                              <button key={name} type="button" onClick={() => toggleOption('skills', name, 7)} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${form.skills[name] ? "bg-purple-600 border-purple-400 text-white shadow-lg" : "bg-white/5 border-transparent text-zinc-500 hover:bg-white/10"}`}>
                                ...
                                {form.skills[name] && <span className="font-black text-[10px] bg-white/20 px-1.5 py-0.5 rounded-md">Lv.{form.skills[name]}</span>}
                              </button>
                            ))}
                          </div>
                        </div>
